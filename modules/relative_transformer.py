...
class RelativeMultiHeadAttn(nn.Module):
    def __init__(self, d_model, n_head, dropout, r_w_bias=None, r_r_bias=None, scale=False):
        ...
        :param int d_model:
        :param int n_head:
        :param dropout: attention map  dropout
        :param r_w_bias: n_head x head_dim or None, dim
    ...
    def forward(self, x, mask):
        ...
        D_ = torch.einsum('nd,ld->nl', self.r_w_bias, pos_embed)[None, :, None]  # head x 2max_len
        B_ = torch.einsum('bnqd,ld->bnql', q, pos_embed)  # bsz x head  x max_len x 2max_len
        BD = B_ + D_  
        BD = self._shift(BD)
        attn = AC + BD
